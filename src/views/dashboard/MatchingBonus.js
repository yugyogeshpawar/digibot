...
import { getMatchingBonus } from '../../redux/admin';
...
export default function MatchingBonus() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const res = await getMatchingBonus();
      console.log('Respons res', res);
      if (Array.isArray(res?.results)) {
        const mappedData = res.results.map((item, index) => ({
          id: index + 1,
          member_user_id: item.member_user_id,
          member_name: item.member_name,
          walletAddress: item.income_amt,
          invest_type: item.income_type,
          matching_bonus: item.net_amt
        }));
        setRows(mappedData);
      }
    };
