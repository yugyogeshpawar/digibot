...
export function getProducts() {
  return async (dispatch) => {
    ...
    try {
      const response = await axios1.get('/api/products');
...
export function getProductsFromServer() {
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.get(`${baseUrl}/Marketplace`, {
        headers
      });
      dispatch(slice.actions.getProductsSuccess(response.data.products));
    } catch (error) {
...
export function getProduct(name) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios1.get('/api/products/product', {
        params: { name }
      });
...
export function getProductFormServer(id) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const accessToken = window.localStorage.getItem('accessToken');
      const headers = { Authorization: `Bearer ${accessToken}` };
      const response = await axios.get(`${baseUrl}/Marketplace/nft/${id}`, {
        headers
      });
      dispatch(slice.actions.getProductSuccess(response.data.product));
    } catch (error) {
