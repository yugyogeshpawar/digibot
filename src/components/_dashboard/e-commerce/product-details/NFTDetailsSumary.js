...
import { Box, Link, Stack, Button, Rating, Tooltip, Divider, Typography, FormHelperText } from '@material-ui/core';
// redux
...
export default function ProductDetailsSumary() {
  ...
  const { id, name, price, cover, status, available, priceSale, totalRating, totalReview, inventoryType } = product;
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (inventoryType === 'out_of_stock' || available === 0) {
        ...
        setSubmitting(false);
        handleBuyNow();
        navigate(PATH_DASHBOARD.nftMarket.checkout);
      } catch (error) {
        setSubmitting(false);
      }
    }
